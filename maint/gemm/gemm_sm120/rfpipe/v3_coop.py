...
def coop_gemm(M, N, K, schedule=None, block_M=256, block_N=128, block_K=128,
              num_stages=3, out_dtype=T.bfloat16):
    ...
    # A warpgroup's half of the A tile, in bytes, and of the scale tile in words.
    a_half_bytes = half_M * block_K // 2
    sfa_half_bytes = half_M * wpk * 4
    scale_tile_words_a = block_M * wpk
    scale_tile_words_b = block_N * wpk
    ...
    def main(
        ...
        with T.Kernel(T.ceildiv(N, block_N), T.ceildiv(M, block_M), threads=384) as (bx, by):
            ...
            # The producer fills a stage; both consumer warpgroups release it.
            ...
            if tx >= 256:
                ...
                # T.clear, no epilogue, so that warpgroup's half of C is never
                # written. Swapping which wg index goes in which branch moves
                # the failure with the branch, not with the index. The
                # production kernel writes both consumer bodies out for the
                # same reason.
                T.clear(C0_local)
                for ko in T.serial(k_blocks):
                    stage = ko % num_stages
                    ...
                    _ = [_emit(op, {
                        ...
                        "C_local": C0_local,
                        "a_off": stage * a_stage_bytes + 0 * a_half_bytes,
                        "b_off": stage * b_stage_bytes,
                        "sfa_off": stage * sfa_stage_bytes + 0 * sfa_half_bytes,
                ...
                for ko in T.serial(k_blocks):
                    ...
                    _ = [_emit(op, {
                        ...
                        "SFA_shared": SFA_shared, "SFB_shared": SFB_shared,
                        "C_local": C1_local,
                        "a_off": stage * a_stage_bytes + 1 * a_half_bytes,
                        "b_off": stage * b_stage_bytes,
                        "sfa_off": stage * sfa_stage_bytes + 1 * sfa_half_bytes,
                        "sfb_off": stage * sfb_stage_bytes,
                        "loaded": loaded, "consumed": consumed,
                        "stage": stage, "phase": phase,
                    }) for op in schedule]
                T.copy(C1_local,
                       C[by * block_M + 1 * half_M:by * block_M + (1 + 1) * half_M,
                         bx * block_N:(bx + 1) * block_N])
